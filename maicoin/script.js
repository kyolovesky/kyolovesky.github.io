  ...
  fetch(`${API_URL}${market}`)
    ...
    .then(data => {
      ...
      if (fromCurrency === 'BTC' && toCurrency === 'USDT') {
        ...
        toAmountInput.value = usdAmount.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      } else {
...
function updateResult(exchangeRate) {
  ...
  if (fromCurrency === 'TWD') {
    ...
    if (toCurrency === 'BTC') {
      ...
      toAmountInput.value = btcAmount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 6 });
      convertedAmount.textContent = `${fromAmountInput.value} ${fromCurrency} = ${toAmountInput.value} ${toCurrency}`;

    } else if (toCurrency === 'USDT') {
      const usdtPrice = exchangeRate;
      const usdtAmount = twdAmount / usdtPrice;
      toAmountInput.value = usdtAmount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 3 });
      convertedAmount.textContent = `${fromAmountInput.value} ${fromCurrency} = ${toAmountInput.value} ${toCurrency}`;
    ...
    if (toCurrency === 'TWD') {
      ...
      toAmountInput.value = twdAmount.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 1 });
      convertedAmount.textContent = `${fromAmountInput.value} ${fromCurrency} = ${toAmountInput.value} ${toCurrency}`;
    ...
    if (toCurrency === 'TWD') {
      ...
      toAmountInput.value = twdAmount.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      convertedAmount.textContent = `${fromAmountInput.value} ${fromCurrency} = ${toAmountInput.value} ${toCurrency}`;

    } else if (toCurrency === 'BTC') {
      const btcPrice = exchangeRate;
      const btcAmount = usdtAmount / btcPrice;
      toAmountInput.value = btcAmount.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      convertedAmount.textContent = `${fromAmountInput.value} ${fromCurrency} = ${toAmountInput.value} ${toCurrency}`;
...
function currencySelectHandler() {
  ...
  toCurrencySelect.querySelector(`option[value="${fromCurrency}"]`).disabled = true;
  toCurrencySelect.querySelector(`option[value="${fromCurrency}"]`).hidden = true;
  fromCurrencySelect.querySelector(`option[value="${toCurrency}"]`).disabled = true;
  fromCurrencySelect.querySelector(`option[value="${toCurrency}"]`).hidden = true;
